 ...
 * edit JSP, the parameters processor, and the mapping discovery. {@link #TYPE} and the parameter
 * keys are persisted in build-config settings, so they must not change without a migration.
 */
public final class JenkinsBridgeBuildFeatureConstants {
  ...
  private JenkinsBridgeBuildFeatureConstants() {
  }
}
